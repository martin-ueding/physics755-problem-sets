...
element from the time ordering. \textcite[90]{Peskin/QFT/1995} define the
numbering of the fields such that $x_k^0 \geq x_{k+1}^0$, such that they are in
time order already. If we do not use this trick as well, we would have $n+1$
terms when we write down the time ordering using Heaviside step functions
explicitly. Therefore we will also use this trick. This means that $\phi_{n+1}$
has the smallest time of all and already is in the correct time order when it
is at the very end.
\begin{align*}
